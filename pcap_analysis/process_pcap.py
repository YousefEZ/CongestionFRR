...
def packet_transfer_time(sender_packets, receiver_packets):
    ...
    for sender_packet in sender_packets:
        for receiver_packet in receiver_packets:
            if (sender_packet.getlayer('TCP').seq == receiver_packet.getlayer('TCP').seq) and (sender_packet.getlayer('TCP').ack == receiver_packet.getlayer('TCP').ack) and (sender_packet.getlayer('IP').src == receiver_packet.getlayer('IP').src) and (sender_packet.getlayer('IP').dst == receiver_packet.getlayer('IP').dst) and (sender_packet.getlayer('TCP').window == receiver_packet.getlayer('TCP').window) and (sender_packet.getlayer('TCP').options == receiver_packet.getlayer('TCP').options) and (sender_packet.getlayer('TCP').options[0][1] == receiver_packet.getlayer('TCP').options[0][1]): 
