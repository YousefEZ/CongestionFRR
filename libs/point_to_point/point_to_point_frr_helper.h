...
class PointToPointFRRHelper : public PcapHelperForDevice, public AsciiTraceHelperForDevice
{
  public:
...
template <typename... Ts>
void
PointToPointFRRHelper::SetQueue(std::string type, Ts&&... args)
{
...
PointToPointFRRHelper::PointToPointFRRHelper()
{
    ...
    m_deviceFactory.SetTypeId("ns3::PointToPointFRRNetDevice");
    m_channelFactory.SetTypeId("ns3::PointToPointFRRChannel");
    m_enableFlowControl = true;
}
 
void
PointToPointFRRHelper::SetDeviceAttribute(std::string n1, const AttributeValue& v1)
{
    m_deviceFactory.Set(n1, v1);
}
 
void
PointToPointFRRHelper::SetChannelAttribute(std::string n1, const AttributeValue& v1)
{
    m_channelFactory.Set(n1, v1);
}
 
void
PointToPointFRRHelper::DisableFlowControl()
{
    m_enableFlowControl = false;
}
 
void
PointToPointFRRHelper::EnablePcapInternal(std::string prefix,
                                       Ptr<NetDevice> nd,
                                       bool promiscuous,
                                       bool explicitFilename)
{
    ...
    // the system.  We can only deal with devices of type PointToPointFRRNetDevice.
    //
    Ptr<PointToPointFRRNetDevice> device = nd->GetObject<PointToPointFRRNetDevice>();
    if (!device)
    {
        NS_LOG_INFO("Device " << device << " not of type ns3::PointToPointFRRNetDevice");
        return;
    }
    ...
    pcapHelper.HookDefaultSink<PointToPointFRRNetDevice>(device, "PromiscSniffer", file);
}
 
void
PointToPointFRRHelper::EnableAsciiInternal(Ptr<OutputStreamWrapper> stream,
                                        std::string prefix,
                                        Ptr<NetDevice> nd,
                                        bool explicitFilename)
{
    ...
    if (!stream)
    {
        ...
        //
        asciiTraceHelper.HookDefaultReceiveSinkWithoutContext<PointToPointFRRNetDevice>(device,
                                                                                     "MacRx",
                                                                                     theStream);
        ...
        asciiTraceHelper.HookDefaultDropSinkWithoutContext<PointToPointFRRNetDevice>(device,
                                                                                  "PhyRxDrop",
                                                                                  theStream);
...
NetDeviceContainer
PointToPointFRRHelper::Install(NodeContainer c)
{
    NS_ASSERT(c.GetN() == 2);
...
NetDeviceContainer
PointToPointFRRHelper::Install(Ptr<Node> a, Ptr<Node> b)
{
    NetDeviceContainer container;
 
    Ptr<PointToPointFRRNetDevice> devA = m_deviceFactory.Create<PointToPointFRRNetDevice>();
    devA->SetAddress(Mac48Address::Allocate());
    a->AddDevice(devA);
    Ptr<Queue<Packet>> queueA = m_queueFactory.Create<Queue<Packet>>();
    devA->SetQueue(queueA);
    Ptr<PointToPointFRRNetDevice> devB = m_deviceFactory.Create<PointToPointFRRNetDevice>();
    ...
    else
    {
        ...
        mpiRecA->SetReceiveCallback(MakeCallback(&PointToPointFRRNetDevice::Receive, devA));
        mpiRecB->SetReceiveCallback(MakeCallback(&PointToPointFRRNetDevice::Receive, devB));
...
NetDeviceContainer
PointToPointFRRHelper::Install(Ptr<Node> a, std::string bName)
{
...
NetDeviceContainer
PointToPointFRRHelper::Install(std::string aName, Ptr<Node> b)
{
...
NetDeviceContainer
PointToPointFRRHelper::Install(std::string aName, std::string bName)
{
    Ptr<Node> a = Names::Find<Node>(aName);
    Ptr<Node> b = Names::Find<Node>(bName);
    return Install(a, b);
